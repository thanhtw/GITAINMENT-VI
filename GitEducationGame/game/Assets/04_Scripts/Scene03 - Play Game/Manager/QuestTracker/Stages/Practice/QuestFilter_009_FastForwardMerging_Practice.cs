...
public class QuestFilter_009_FastForwardMerging_Practice : SerializedMonoBehaviour
{
        ...
        if (Sender.CompareTag("Window/CommandLineWindow/InputField"))
        {
            ...
            if (commandActionDict.ContainsKey(commandType))
            {
                ...
                switch (commandType)
                {
                    case "branch":
                        if (splitList.Length == 4)
                        {
                            ...
                            if (splitList[2] == "-d" || splitList[2] == "--delete")
                            {
                                ...
                                {
                                    case 7:
                                        return questFilterManager.DetectAction_GitDeleteLocalBranch(splitList[3], "new-character-action");
                                    default:
                    ...
                    case "merge":
                        //Todo
                        if (splitList.Length == 3 && foundIndex != -1 && currentQuestNum == 6) //Give warning (use 'git log' first).
                        {
                            //Fast Forward
                            string resultText = questFilterManager.DetectAction_GitMerge(splitList[2], "master", "new-character-action", false, isMergeConflict);
                        ...
                        {
                            return (currentQuestNum == 6) ? "Continue" : "Git Commands/common/FollowQuest(Warning)";
                        }
                    default:
                        return "Continue";
                }
            ...
            else
            {
                switch (currentQuestNum)
                {
                    case 3:
                        ...
                        {
                            return questFilterManager.DetectAction_AddContentFile(i18nTranslateList[0], "new-character-action");
                        }
                        else
                        {
                            return $"{Sender.tag}/Wrong Quest";
