...
public class QuestFilter_008_GitBranchingBasics_Practice : SerializedMonoBehaviour
{
                    ...
                    case "branch":
                        if (splitList.Length == 4)
                        {
                            ...
                            if (splitList[2] == "-d" || splitList[2] == "--delete")
                            {
                                ...
                                {
                                    case 9:
                                        return questFilterManager.DetectAction_GitDeleteLocalBranch(splitList[3], "new-test-level");
                                    default:
            ...
            else
            {
                switch (currentQuestNum)
                {
                    case 6:
                        ...
                        {
                            return questFilterManager.DetectAction_AddContentFile(i18nTranslateList[0], "new-test-level");
                        }
                        else
                        {
                            return $"{Sender.tag}/Wrong Quest";
